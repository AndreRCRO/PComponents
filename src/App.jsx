...
import { brands, categories, filterProducts, formatPrice, products } from './catalog.js'
...
function Header({ query, setQuery, cartCount, path, onCartOpen }) {
...
function FilterSidebar({ filters, setFilters, onClose }) {
  ...
  const activeCount = filters.categories.length + filters.brands.length + (filters.maxPrice < 6000 ? 1 : 0)
  return <aside className="filter-sidebar" aria-label="Filtros de productos">
    ...
    <fieldset className="price-filter"><legend>Precio máximo</legend><div><strong>{formatPrice(filters.maxPrice)}</strong><span>Bs 6.000</span></div><input type="range" min="500" max="6000" step="100" value={filters.maxPrice} onChange={event => setFilters(current => ({ ...current, maxPrice: Number(event.target.value) }))} aria-label="Precio máximo"/><div className="range-labels"><span>Bs 500</span><span>Bs 6.000</span></div></fieldset>
    <button className="clear-filters" onClick={() => setFilters({ categories: [], brands: [], maxPrice: 6000 })}>Limpiar filtros</button>
  </aside>
}
...
const homeCategories = [
  ['Tarjetas gráficas', 'Tarjetas gráficas', '/products/asus-rtx4060.webp'],
  ['Procesadores', 'Procesadores', '/products/amd-ryzen7.jpg'],
  ['Memorias RAM', 'RAM', '/products/kingston-fury.png'],
  ['SSD', 'SSD', '/products/samsung-990pro.png'],
  ['Monitores', 'Monitores', '/products/asus-vg27.webp'],
  ['Periféricos', 'Periféricos', '/products/logitech-g733-lilac.webp'],
]
...
const heroSlides = [
  ...
  { eyebrow: 'Gráficos de nueva generación', title: 'Potencia para jugar en 1080p', text: 'ASUS GeForce RTX 4060 con DLSS 3 y refrigeración de doble ventilador.', image: '/products/asus-rtx4060.webp', alt: 'Tarjeta gráfica ASUS GeForce RTX 4060', to: '/producto/asus-rtx-4060-dual', tone: 'blue' },
  { eyebrow: 'Precisión competitiva', title: 'Cada movimiento cuenta', text: 'Logitech G502 X con sensor HERO 25K y switches LIGHTFORCE.', image: '/products/logitech-g502x.png', alt: 'Mouse Logitech G502 X', to: '/producto/logitech-g502-x', tone: 'light' },
  { eyebrow: 'Audio sin cables', title: 'Juega a tu manera', text: 'Logitech G733 LIGHTSPEED disponible en cuatro colores.', image: '/products/logitech-g733-blue.webp', alt: 'Auriculares Logitech G733 color azul', to: '/producto/logitech-g733', tone: 'violet' },
]
...
function HomePage({ onBuy }) {
  ...
  return <main className="home-page">
    <HeroCarousel/>
    <section className="home-section container" id="categorias"><div className="section-heading split"><h2>Encuentra lo que necesitas</h2><Link to="/productos">Ver todo <Icon name="chevron" size={15}/></Link></div><div className="category-accordion">{homeCategories.map(([name, category, image]) => <Link to={`/productos?categoria=${encodeURIComponent(category)}`} key={name}><img src={image} alt="" width="420" height="520" loading="lazy"/><span><strong>{name}</strong><small>Explorar categoría</small></span></Link>)}</div></section>
...
function CatalogPage({ query, setQuery, onBuy }) {
  const categoryParam = new URLSearchParams(window.location.search).get('categoria')
  const defaultFilters = { categories: categories.includes(categoryParam) ? [categoryParam] : [], brands: [], maxPrice: 6000 }
  ...
  return <main className="catalog-page">
    ...
    <section className="catalog container" id="categorias">
      <div className={`filter-drawer ${filtersOpen ? 'open' : ''}`}><FilterSidebar filters={filters} setFilters={setFilters} onClose={() => setFiltersOpen(false)}/></div>
      <div className={`overlay filter-overlay ${filtersOpen ? 'show' : ''}`} onClick={() => setFiltersOpen(false)}/>
      <div className="desktop-filters"><FilterSidebar filters={filters} setFilters={setFilters}/></div>
      <div className="catalog-results">
        ...
        <ProductGrid items={visibleProducts} onBuy={onBuy} onReset={() => { setFilters({ categories: [], brands: [], maxPrice: 6000 }); setQuery('') }}/>
      </div>
    </section>
...
function ProductGallery({ product, variant }) {
  const images = variant ? [variant.image] : product.gallery || [product.image]
...
function ProductInfo({ product, variant, onVariantChange, onBuy }) {
  return <div className="product-info"><span className="product-brand">{product.brand}</span><h1>{product.name}</h1><p className="product-summary">{product.description}</p><dl><div><dt>Marca</dt><dd>{product.brand}</dd></div><div><dt>Categoría</dt><dd>{product.category}</dd></div><div><dt>Disponibilidad</dt><dd className="in-stock"><span/>Disponible</dd></div></dl>{product.variants && <fieldset className="variant-picker"><legend>Color: <strong>{variant.name}</strong></legend><div>{product.variants.map(option => <button key={option.name} className={variant.name === option.name ? 'selected' : ''} onClick={() => onVariantChange(option)} aria-label={`Seleccionar color ${option.name}`} aria-pressed={variant.name === option.name}><span style={{ background: option.color }}/>{option.name}</button>)}</div></fieldset>}<div className="detail-price"><small>Precio</small><strong>{formatPrice(product.price)}</strong></div><div className="detail-actions"><button className="button button-primary" onClick={() => onBuy({ ...product, name: variant ? `${product.name} ${variant.name}` : product.name })}><Icon name="cart"/>Agregar al carrito</button><a className="button button-secondary" href={whatsappLink(variant ? `${product.name} color ${variant.name}` : product.name)} target="_blank" rel="noreferrer"><Icon name="whatsapp"/>Consultar por WhatsApp</a></div></div>
}
...
function PerformanceGuide({ performance }) {
  if (!performance) return null
  const [game, fps, preset] = performance.featured
  return <section className="performance-guide" id="rendimiento"><div className="performance-heading"><div><h2>Rendimiento orientativo en juegos</h2><p>Configuración de referencia: {performance.system}</p></div><span>Estimación</span></div><div className="performance-layout"><article className="performance-featured"><span>{game}</span><strong>{fps}</strong><small>{preset}</small></article><div className="performance-list">{performance.games.map(([title, result, quality]) => <div key={title}><span><strong>{title}</strong><small>{quality}</small></span><b>{result}</b></div>)}</div></div><p className="performance-note">Los FPS son valores orientativos y pueden variar según procesador, memoria, controladores, mapa y ajustes del juego.</p></section>
}

function RelatedProducts({ product, onBuy }) {
...
function ProductDetailPage({ id, onBuy }) {
  ...
  return <main className="product-detail-main"><div className="product-page container"><Breadcrumb items={[product.category, product.name]}/><section className="product-detail"><ProductGallery product={product} variant={variant}/><ProductInfo product={product} variant={variant} onVariantChange={setVariant} onBuy={onBuy}/></section><section className={`product-copy${product.performance ? '' : ' no-performance'}`}><div className="product-description"><h2>Diseñado para rendir</h2><p>{product.longDescription}</p><ProductGuide product={product}/></div><div className="product-specification"><h2>Especificaciones</h2><dl className="spec-grid">{product.specs.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></div></section><PerformanceGuide performance={product.performance}/></div><RelatedProducts product={product} onBuy={onBuy}/><div className="mobile-buy-bar"><div><small>{product.name}</small><strong>{formatPrice(product.price)}</strong></div><button className="button button-primary" onClick={() => onBuy(selectedProduct)}><Icon name="cart" size={18}/>Agregar</button></div></main>
}
...
function App() {
  ...
  const addProductsToCart = (productsToAdd, message) => {
    setCartItems(items => {
      ...
      productsToAdd.forEach(product => {
        ...
        else next.push({ key, id: product.id, name: product.name, image: product.image, price: product.price, quantity: 1, bundleDiscount: product.bundleDiscount })
      })
      return next
    })
    setToast(message)
  ...
  const changeQuantity = (key, amount) => setCartItems(items => items.map(item => item.key === key ? { ...item, quantity: Math.max(1, item.quantity + amount) } : item))
  ...
  return <>
    ...
    <Header query={query} setQuery={setQuery} cartCount={cartCount} path={path} onCartOpen={() => setCartOpen(true)}/>
    <div id="contenido">{productMatch ? <ProductDetailPage id={productMatch[1]} onBuy={buy}/> : path === '/productos' ? <CatalogPage query={query} setQuery={setQuery} onBuy={buy}/> : path === '/arma-tu-pc' ? <PCBuilderPage onNavigate={navigate} onAddBuild={items => addProductsToCart(items, 'Tu configuración fue añadida al carrito')}/> : <HomePage onBuy={buy}/>}</div>
    <Footer/>
